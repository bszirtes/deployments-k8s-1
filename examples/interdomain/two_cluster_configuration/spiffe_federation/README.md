...
```bash
kubectl --kubeconfig=$KUBECONFIG1 apply -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/v0.1.10/examples/interdomain/two_cluster_configuration/spiffe_federation/cluster1-spiffeid-template.yaml
```

Apply the ClusterSPIFFEID CR for the second cluster:
```bash
kubectl --kubeconfig=$KUBECONFIG2 apply -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/v0.1.10/examples/interdomain/two_cluster_configuration/spiffe_federation/cluster2-spiffeid-template.yaml
```
